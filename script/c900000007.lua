...
function c900000007.initial_effect(c)
	...
	c:SetCounterLimit(0x1,2) -- 🔧 FIX : obligatoire pour les cartes custom

	-- Place Spell Counter (max 2)
	...
	-- Tribute with 2 Spell Counters to Special Summon "Buster Blader" monsters
	...
	-- Banish from GY to add 1 Spell Counter elsewhere
...
function c900000007.spfilter(c,e,tp)
	return c:IsCode(78193831) or c:IsCode(78193832) or c:IsCode(78193833) -- mets les bons ID
		and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end

function c900000007.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then 
		return Duel.IsExistingMatchingCard(c900000007.spfilter,tp,LOCATION_HAND+LOCATION_DECK+LOCATION_GRAVE,0,1,nil,e,tp) 
	end
...
function c900000007.ctfilter(c)
	return c:IsCanAddCounter(0x1,1)
end

function c900000007.cttg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c900000007.ctfilter,tp,LOCATION_ONFIELD,0,1,nil) end
end

function c900000007.ctop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_COUNTER)
	local g=Duel.SelectMatchingCard(tp,c900000007.ctfilter,tp,LOCATION_ONFIELD,0,1,1,nil)
	local tc=g:GetFirst()
