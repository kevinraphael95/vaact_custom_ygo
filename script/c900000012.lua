...
--Targeting
function c900000012.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsControler(1-tp) and chkc:IsLocation(LOCATION_MZONE) and c900000012.filter(chkc) end
	if chk==0 then return Duel.CheckLPCost(tp,800)
		and Duel.IsExistingTarget(c900000012.filter,tp,0,LOCATION_MZONE,1,nil) end
	Duel.PayLPCost(tp,800)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_CONTROL)
	local g=Duel.SelectTarget(tp,c900000012.filter,tp,0,LOCATION_MZONE,1,1,nil)
